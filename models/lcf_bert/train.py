...
def evaluate_model(model: LCF_BERT, loader: DataLoader, device: torch.device) -> dict[str, float]:
    ...
    for batch in loader:
        ...
        asp_gold.extend(batch["aspect_label"].tolist())
        sent_pred.extend(sent_logits.argmax(dim=-1).cpu().tolist())
        asp_pred.extend(asp_logits.argmax(dim=-1).cpu().tolist())

    joint = sum(
        sg == sp and ag == ap
        for sg, sp, ag, ap in zip(sent_gold, sent_pred, asp_gold, asp_pred)
    )
    n = max(len(sent_gold), 1)
    return {
        ...
        "aspect_accuracy": accuracy_score(asp_gold, asp_pred) if asp_gold else 0.0,
        "aspect_macro_f1": f1_score(asp_gold, asp_pred, average="macro", zero_division=0) if asp_gold else 0.0,
    ...
    torch.save(
        {
            ...
            "opt": model_config,  # backward-compatible key for old predictor code
...
def train(cfg: SimpleNamespace) -> Path:
    ...
    for epoch in range(1, int(cfg.num_epoch) + 1):
        ...
        logger.info(
            "Epoch %d/%d | sent_acc=%.4f sent_f1=%.4f asp_acc=%.4f asp_f1=%.4f joint=%.4f",
            epoch,
            int(cfg.num_epoch),
            metrics["sentiment_accuracy"],
            metrics["sentiment_macro_f1"],
            metrics["aspect_accuracy"],
            metrics["aspect_macro_f1"],
