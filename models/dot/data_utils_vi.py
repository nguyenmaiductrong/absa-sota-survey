...
def get_transformed_io(data_path, data_name, data_type, args, n_tuples, wave):
    ...
    cache_file = os.path.join(cache_dir, f"cached_{data_name}_{data_type}.pkl")
    
    if os.path.exists(cache_file):
        print(f"✅ LOAD CACHE: Tìm thấy {cache_file}. Đang tải dữ liệu...")
        with open(cache_file, 'rb') as f:
            cached_data = pickle.load(f)
        new_inputs = cached_data['new_inputs']
        targets = cached_data['targets']
        order_targets = cached_data['order_targets']
        print("✅ Đã load xong. Bỏ qua bước tính toán.")
    else:
        print(f"⏳ TÍNH TOÁN: Không tìm thấy Cache. Đang chạy tiền xử lý...")
        if wave == 1:
        ...
        print(f"💾 SAVE CACHE: Đã lưu kết quả vào {cache_file}")
